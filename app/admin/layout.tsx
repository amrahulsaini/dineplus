...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-orange-50">
      ...
      <div className="flex">
        {/* Sidebar */}
        <aside className="w-64 bg-white shadow-xl min-h-[calc(100vh-4rem)] border-r-2 border-orange-200">
        ...
        {/* Main Content */}
        <main className="flex-1 p-8 bg-gradient-to-br from-gray-50 to-orange-50">
          {children}
        </main>
      </div>
    </div>
...
function NavLink({ href, icon, children }: { href: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="flex items-center gap-3 px-4 py-3 rounded-2xl hover:bg-gradient-to-r hover:from-orange-500/10 hover:to-red-500/10 hover:text-orange-600 text-gray-700 transition-all duration-300 group border-2 border-transparent hover:border-orange-300 hover:shadow-lg transform hover:translate-x-1"
    >
      <span className="text-orange-500 group-hover:text-orange-600 transition-colors duration-300">{icon}</span>
      <span className="font-semibold group-hover:text-orange-600 transition-colors duration-300">{children}</span>
    </Link>
  );
}
